...
class MobileNetV2(nn.Module):
    def __init__(self, width_mult=1., last_channel=1280, act='ReLU'):
    ...
    def build_mobilenet_stage(self, stage_num):
        ...
        for i in range(n):
            if i == 0:
                stage.append(InvertedResidual(self.input_channel,
                             output_channel, s, expand_ratio=t, act=self.act))
            else:
                stage.append(InvertedResidual(self.input_channel,
                             output_channel, 1, expand_ratio=t, act=self.act))
